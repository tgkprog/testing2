...
#sleep 2
TOMCAT_A=/data/ddt/tomcatAgent
TOMCAT_C=/data/ddt/tomcatCore
...
cp -v $BLD_HOME/Utils/target/DdtUtils-0.0.1.jar .
echo "Utils done"
sleep 2

tar -zxvf $BLD_HOME/Data/target/DdtData*-default.tar.gz
mv DdtData-0.0.1/lib/* .
rm  -rf DdtData-0.0.1
## cp -v $BLD_HOME/Data/target/DdtData-0.0.1.jar .
echo "Data done"
sleep 3

tar -zxvf $BLD_HOME/Verifiers/target/Verifiers*-default.tar.gz
mv Verifiers-1/lib/* .
rm  -rf Verifiers-1
## cp -v $BLD_HOME/Verifiers/target/Verifiers-1.jar .
echo "Verifiers done"
sleep 2
...
## rm  -rf DdtCore-0.0.1
cp -v $BLD_HOME/Core/target/DdtCore-0.0.1.jar .
echo "Core done"
sleep 3
...
cp -v $BLD_HOME/Agent/target/DdtAgent-0.0.1.jar .
echo "Agent done"
sleep 3
...
tar -zxvf $BLD_HOME/SquishRunner/target/SQ-0.0.1-SNAPSHOT-default.tar.gz
mv SQ-0.0.1-SNAPSHOT/lib/* .
rm -rf SQ-0.0.1-SNAPSHOT
echo "SquishRunner done"
sleep 3
...
cd   $BLD_HOME/ddtDeploy/tmp
cp *.* $TOMCAT_A/shared/lib
echo "Copied to tomcat agent $TOMCAT_A/shared/lib"

cp *.* $TOMCAT_C/shared/lib
echo "Copied to tomcat core $TOMCAT_C/shared/lib"
sleep 5
