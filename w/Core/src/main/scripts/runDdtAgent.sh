...
if [ "x$JAVAC_JAR_FILE" != "x" ]; then
 ...
 li=$JBOSS_HOME/server/default/lib
 JBOSS_CLASSPATH="$JBOSS_CLASSPATH:$li/DdtUtils-0.0.1.jar:$li/DdtData-0.0.1.jar:$li/DdtCore-0.0.1.jar:$li/DdtAgent-0.0.1.jar:$li/ApiRunner-1.jar:$li/SA-1.jar:$li/HttpRunner-1.jar"
...
#comment out next line by putting a # as char 1 if you do not want to debug from IDE. To debug Core and agent, put similar line in Core run.sh with a different port
JAVA_OPTS="-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=1045 $JAVA_OPTS"

JAVA_OPTS="-Dlog4j.configuration=/data/ddt/config/log4ddt.xml $JAVA_OPTS"

# JAVA_OPTS="-verbose $JAVA_OPTS"
