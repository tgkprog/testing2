...
public class ReportHelper {
	...
	public static void htmlReport(List<TestSuiteResult> suiteResults, AgentRunResult agentRR, Functional functionalList) throws Exception {
		...
		functionalList
				.setPercentageNotExecuted(Math.round((float) ((100 * ((double) functionalList.getNotExecutedCount() / (double) functionalList.getTotalCount()) * 100) / 100)));
		...
		while (iterator2.hasNext()) {
			...
			testSuiteResult
					.setPercentagePassCount(Math.round((float) ((100 * ((double) testSuiteResult.getPassCount() / (double) testSuiteResult.getTotalCount()) * 100) / 100)));
			testSuiteResult
					.setPercentageFailCount(Math.round((float) ((100 * ((double) testSuiteResult.getFailCount() / (double) testSuiteResult.getTotalCount()) * 100) / 100)));
			testSuiteResult
					.setPercentageSkipCount(Math.round((float) ((100 * ((double) testSuiteResult.getSkipCount() / (double) testSuiteResult.getTotalCount()) * 100) / 100)));
			testSuiteResult
					.setPercentageNotExecuted(Math.round((float) ((100 * ((double) testSuiteResult.getNotExecuted() / (double) testSuiteResult.getTotalCount()) * 100) / 100)));

			System.out.println("resultList >>" + suiteResults.size());
			logger.info("resultList >>" + agentRR.getAgentName());
		...
		p.setProperty("file.resource.loader.path", "/data/ddt/res/velocity/resources");
		ve.init(p);
		...
		File root = agentRR.getReportFilePath().getParentFile();
		File file = new File(root, agentRR.getAgentName() + "Report.html");
		logger.info("In Report Helper the agent Name =="+agentRR.getAgentName());
		...
		logger.info(" vm done :" + agentRR.getReportFilePath());
		...
		while (iterator3.hasNext()) {
			...
			File file1 = new File(root, testSuiteResult.getSuiteName() + "_" + agentRR.getAgentName() + ".html");
