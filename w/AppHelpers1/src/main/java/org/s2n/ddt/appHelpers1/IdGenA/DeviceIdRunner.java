package org.s2n.ddt.appHelpers1.IdGenA;
...
public class DeviceIdRunner extends RunnerAdapter implements Runner, Runner2 {
	...
	// private org.s2n.ddt.grxHelpers.DeviceIdGenerator dig = new
	// DeviceIdGenerator();

	private Map<String, IdGen> digs = new HashMap<String, IdGen>(1);

	public void testStep(TestPlan obj, TestSuite suite, TestCase tcase,
			TestCaseResult caseResult, TestStep step,
			TestStepResult stepResult, TestSuiteResultSummary suiteSumamryResult) {
		genOne(obj, suite, tcase, caseResult, step, stepResult,
				suiteSumamryResult);
	}

	// DivIdSimpleRange

	public void genOne(TestPlan obj, TestSuite suite, TestCase tcase,
			TestCaseResult caseResult, TestStep step,
			TestStepResult stepResult, TestSuiteResultSummary suiteSumamryResult) {
		...
		String summary = "";
		UtlProps props;
		synchronized (DeviceIdRunner.class) {
